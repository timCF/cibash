...
function deploy_upload {
	local UPLOAD_URL=$(curl -H "Authorization: OAuth $YANDEX_TOKEN" "https://cloud-api.yandex.net/v1/disk/resources/upload?fields=href&overwrite=true&path=%2Felixir_releases%2F$RELEASE_FILENAME" | sed -e 's/^.*"href"[ ]*:[ ]*"//' -e 's/".*//') &&
