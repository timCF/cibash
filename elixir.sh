...
function deploy {
	...
	ssh -o StrictHostKeyChecking=no -i $ID_RSA_FILENAME $RELEASE_SERVER "cd $RELEASES_DIR/$ERLANG_NODE && tar xvfz $ERLANG_NODE.tar.gz && sed -i 's/-sname\ $THIS_APP/-sname\ $ERLANG_NODE@localhost/g' ./releases/0.1.0/vm.args $DEPLOY_END"
}
